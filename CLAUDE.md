# VindJeFysio Netwerk — heupbovenbeen.net (Heup Bovenbeen Netwerk)

Deze repo is één "spoke" in een hub-and-spoke netwerk van gespecialiseerde fysiotherapie-subsites. De hub is vindjefysio.net. Deze site hoort daarnaast, samen met enkelvoet.net en knieonderbeen.net, onder de mid-level hub beenklachten.net (cluster "Benen").
...
- Palet identiek aan enkelvoet.net, knieonderbeen.net en beenklachten.net (bewust gedeeld binnen het "Benen"-cluster): navy #1B3A5C, teal #2A9D8F, teal-light #E8F5F4.
...
- locatie-worden.html is een uitlegpagina die doorlinkt naar het centrale vindjefysio.net/aanmelden.html?via=heupbovenbeen.net.
...
- Deze site: subcategorie 43 (Heup, lies en bovenbeen) is het primaire domein; 46 (Artroseprogramma's), 47 (Revalidatie na operatie), 48 (Voorbereiding op operatie), 49 (Sport, bewegen en overbelasting) zijn de gedeelde been-thema's (categorie "Benen" in Supabase, subcats 43–49, zie ook beenklachten.net).
...
- Cursusplatform: nog geen content. `MODULES = []` in mijn-profiel.html, met een "binnenkort"-lijst van placeholder-modules voor heup/bovenbeen. Cursustoegang blijft, zoals bij enkelvoet.net, tier-gated (alleen praktijken met tier plus/partner).

## Sync-pipeline
- sync_locaties.py haalt actieve praktijken/therapeuten op uit Supabase, filtert (ANDERS dan enkelvoet.net) op subcategorie-koppeling in de set [43, 46, 47, 48, 49], geocodeert via Nominatim, schrijft locaties.json. Workflow git-add: locaties.json.
...
- Zones (protocollen-config.json): heup, lies, bovenbeen.
